...
deprecated("This function was accidentally exposed and should not be used") void removeFromArray(T)(
		ref T[] array, T item) {
...
deprecated("This function was accidentally exposed and should not be used") void removeFromArrayIdx(
		T)(ref T[] array, size_t idx) {
...
struct FixedRingBuffer(T, size_t N = 0, bool INITIALIZE = true) {
	...
	@property size_t freeSpace() const {
		return m_buffer.length - m_fill;
	}
	...
	@property ref inout(T) back() inout {
		assert(!empty);
		return m_buffer[mod(m_start + m_fill - 1)];
	}
	...
	void putBack()(T itm) {
		...
		m_buffer[mod(m_start + m_fill++)] = itm;
	}
	...
	void putBackN(size_t n) {
		assert(m_fill + n <= m_buffer.length);
		m_fill += n;
	}
	...
	inout(T)[] peek() inout {
		return m_buffer[m_start .. min(m_start + m_fill, m_buffer.length)];
	}

	T[] peekDst() {
		if (m_start + m_fill < m_buffer.length)
			return m_buffer[m_start + m_fill .. $];
		else
			return m_buffer[mod(m_start + m_fill) .. m_start];
	}
	...
	ref inout(T) opIndex(size_t idx) inout {
		assert(idx < length);
		return m_buffer[mod(m_start + idx)];
	}
...
unittest {
	...
	buf.put(1); // |1 . . . .
	assert(buf.length == 1 && buf.freeSpace == 4);
	buf.put(2); // |1 2 . . .
	assert(buf.length == 2 && buf.freeSpace == 3);
	buf.put(3); // |1 2 3 . .
	assert(buf.length == 3 && buf.freeSpace == 2);
	buf.put(4); // |1 2 3 4 .
	assert(buf.length == 4 && buf.freeSpace == 1);
	buf.put(5); // |1 2 3 4 5
...
struct ArraySet(Key) {
	...
	~this() @trusted {
		if (m_entries.ptr) {
			if (--allocator.prefix(m_entries) <= 0) {
				try
					allocator.dispose(m_entries);
				catch (Exception e)
					assert(false, e.msg); // should never happen
			}
		}
	}
	...
	@property ArraySet dup() {
		...
		if (m_entries.length) {
			...
			() @trusted {
				try
					duped = allocator.makeArray!(Key)(m_entries.length);
				catch (Exception e)
					assert(false, e.msg);
				if (!duped.length)
	...
	in {
		assert(m_entries.ptr is null, "Cannot set allocator after elements have been inserted.");
	}
	do {
	...
	ref allocator() nothrow @trusted {
		try {
			...
			assert(false, e.msg); // should never throw
		return m_allocator;
	}
}
