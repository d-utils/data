...
*/
module dutils.data.utils.serialization;
...
import dutils.data.utils.traits;
import dutils.data.utils.uda;
...
/** Note that there is a convenience function `dutils.data.json.serializeToJSON`
...
unittest {
	...
	template SizePol(T) if (__traits(allMembers, T) == TypeTuple!("x", "y")) {
		import std.conv;
		import std.array;
...
/** Note that there is a convenience function `dutils.data.json.deserializeJSON`
...
private template serializeValueImpl(Serializer, alias Policy) {
	...
	static assert(Serializer.isSupportedValueType!string,
			"All serializers must support string values.");
	static assert(Serializer.isSupportedValueType!(typeof(null)),
			"All serializers must support null values.");

	// work around https://issues.dlang.org/show_bug.cgi?id=16528
	static if (isSafeSerializer!Serializer) {
		void serializeValue(T, ATTRIBUTES...)(ref Serializer ser, auto ref T value) @safe {
			serializeValueDeduced!(T, ATTRIBUTES)(ser, value);
		}
	} else {
		void serializeValue(T, ATTRIBUTES...)(ref Serializer ser, auto ref T value) {
			serializeValueDeduced!(T, ATTRIBUTES)(ser, value);
		}
	}
		...
		static if (isPolicySerializable!(Policy, TU)) {
			...
			foreach (key, ref el; value) {
				...
				static if (is(TK : string))
					...
					keyname = key.to!string;
				else static if (isStringSerializable!TK)
					keyname = key.toString();
				else
					static assert(false,
							"Associative array keys must be strings, numbers, enums, or have toString/fromString methods.");
			...
			if (value.isNull())
				...
				ser.serializeValue!(typeof(value.get()), ATTRIBUTES)(value.get());
			...
			static if (hasPolicyAttributeL!(AsArrayAttribute, Policy, ATTRIBUTES)) {
				...
				foreach (mname; SerializableFields!(TU, Policy)) {
					...
					foreach (j, TM; TMS) {
						alias TA = TypeTuple!(__traits(getAttributes,
								TypeTuple!(__traits(getMember, T, mname))[j]));
				...
				static if (__traits(compiles, ser.beginWriteDictionary!Traits(0))) {
					...
					foreach (mname; SerializableFields!(TU, Policy)) {
						static if (!isBuiltinTuple!(T, mname)) {
							...
							static if (is(typeof(vt) : Nullable!NVT, NVT) && hasPolicyAttribute!(EmbedNullableIgnoreNullAttribute,
									Policy, TypeTuple!(__traits(getMember, T, mname))[0])) {
								if (vt.isNull)
									nfields--;
							}
						}
				...
				foreach (mname; SerializableFields!(TU, Policy)) {
					...
					alias TA = TypeTuple!(__traits(getAttributes,
							TypeTuple!(__traits(getMember, T, mname))[0]));
					enum name = getPolicyAttribute!(TU, mname, NameAttribute, Policy)(
								NameAttribute!DefaultPolicy(underscoreStrip(mname))).name;
					static if (!isBuiltinTuple!(T, mname)) {
						auto vtn = safeGetMember!mname(value);
						static if (is(typeof(vtn) : Nullable!NVT, NVT) && hasPolicyAttribute!(EmbedNullableIgnoreNullAttribute,
								Policy, TypeTuple!(__traits(getMember, T, mname))[0])) {
							if (vtn.isNull)
								continue;
							auto vt = vtn.get;
...
private template deserializeValueImpl(Serializer, alias Policy) {
	...
	static if (isSafeDeserializer!Serializer) {
		T deserializeValue(T, ATTRIBUTES...)(ref Serializer ser) @safe {
			return deserializeValueDeduced!(T, ATTRIBUTES)(ser);
		}
	} else {
		T deserializeValue(T, ATTRIBUTES...)(ref Serializer ser) {
			return deserializeValueDeduced!(T, ATTRIBUTES)(ser);
		}
	}
	...
	T deserializeValueDeduced(T, ATTRIBUTES...)(ref Serializer ser) if (isMutable!T) {
		...
		static if (isPolicySerializable!(Policy, T)) {
			...
			static if (hasPolicyAttributeL!(ByNameAttribute, Policy, ATTRIBUTES)) {
				return ser.deserializeValue!(string, ATTRIBUTES)
					.to!T();
			} else {
			...
			static if (all!"!a.empty"([T.fieldNames])
				...
				foreach (i, fieldName; T.fieldNames)
					enforce(set[i],
							"Missing tuple field '" ~ fieldName ~ "' of type '"
							~ T.Types[i].stringof ~ "' (" ~ Policy.stringof ~ ").");
				return ret;
				...
				enforce(currentField == fieldsCount,
						"Missing tuple field(s) - expected '" ~ fieldsCount.stringof
						~ "', received '" ~ currentField.stringof ~ "' (" ~ Policy.stringof ~ ").");
				return ret;
			}
			...
			ser.readArray!Traits((sz) @safe { ret.reserve(sz); }, () @safe {
				...
				static if (__traits(compiles, ()@safe {
						ser.deserializeValue!(TV, ATTRIBUTES);
					}))
			...
			ser.readDictionary!Traits((name) @safe {
				TK key;
				static if (is(TK == string) || (is(TK == enum) && is(OriginalType!TK == string)))
					key = cast(TK) name;
				else static if (is(TK : real) || is(TK : long) || is(TK == enum))
					key = name.to!TK;
				else static if (isStringSerializable!TK)
					key = TK.fromString(name);
				else
					static assert(false,
						"Associative array keys must be strings, numbers, enums, or have toString/fromString methods.");
			...
			static if (hasPolicyAttributeL!(AsArrayAttribute, Policy, ATTRIBUTES)) {
				...
				ser.readArray!Traits((sz) {}, {
					static if (hasSerializableFields!(T, Policy)) {
						...
						pragma(msg,
							"Deserializing composite type " ~ T.stringof ~ " which has no serializable fields.");
					}
				});
			} else {
				ser.readDictionary!Traits((name) {
						...
						default:
							...
							foreach (i, mname; SerializableFields!(T, Policy)) {
								...
								alias TA = TypeTuple!(__traits(getAttributes,
									TypeTuple!(__traits(getMember, T, mname))[0]));
								alias STraits = SubTraits!(Traits, TM, TA);
								enum fname = getPolicyAttribute!(T, mname, NameAttribute, Policy)(
										NameAttribute!DefaultPolicy(underscoreStrip(mname))).name;
						case fname:
								static if (hasPolicyAttribute!(OptionalAttribute, Policy,
									TypeTuple!(__traits(getMember, T, mname))[0]))
									if (ser.tryReadNull!STraits())
										return;
								set[i] = true;
			...
			foreach (i, mname; SerializableFields!(T, Policy))
				static if (!hasPolicyAttribute!(OptionalAttribute, Policy,
						TypeTuple!(__traits(getMember, T, mname))[0]))
					enforce(set[i],
							"Missing non-optional field '" ~ mname ~ "' of type '" ~ T.stringof
							~ "' (" ~ Policy.stringof ~ ").");
			return ret;
...
unittest {
	...
	struct Test {
		...
		@name("print-size")
		@name!CustomPolicy("PRINTSIZE") int printSize;
...
unittest {
	...
	static struct S {
		...
		uint toRepresentation() const {
			return x + (y << 16);
		}

		static S fromRepresentation(uint i) {
			return S(i & 0xFFFF, i >> 16);
		}
	}
...
unittest {
	...
	static struct S {
		...
		static S fromISOExtString(string s) {
			return S.init;
		}
	}
...
private template ChainedPolicy(alias Primary, Fallbacks...) {
	static if (Fallbacks.length == 0) {
		...
		alias ChainedPolicy = ChainedPolicy!(ChainedPolicyImpl!(Primary,
...
private template isSafeSerializer(S) {
	...
	static if (__traits(hasMember, S, "writeValue"))
		enum isSafeSerializer = __traits(compiles, (S s) @safe {
				s.writeValue!T(42);
			});
	else
		static assert(0, "Serializer is missing required writeValue method");
}
...
private template isSafeDeserializer(S) {
	...
	static if (__traits(hasMember, S, "readValue"))
		enum isSafeDeserializer = __traits(compiles, (S s) @safe {
				s.readValue!(T, int)();
			});
	else
		static assert(0, "Deserializer is missing required readValue method");
}
...
private template hasPolicyAttributeImpl(alias T, alias POLICY, alias decl) {
	enum hasPolicyAttributeImpl = hasAttribute!(T!POLICY, decl)
		|| hasAttribute!(T!DefaultPolicy, decl);
}
...
private template hasAttributeL(T, ATTRIBUTES...) {
	static if (ATTRIBUTES.length == 1) {
		...
		enum hasAttributeL = hasAttributeL!(T, ATTRIBUTES[0 .. $ / 2])
			|| hasAttributeL!(T, ATTRIBUTES[$ / 2 .. $]);
	} else {
...
private template hasPolicyAttributeL(alias T, alias POLICY, ATTRIBUTES...) {
	enum hasPolicyAttributeL = hasAttributeL!(T!POLICY, ATTRIBUTES)
		|| hasAttributeL!(T!DefaultPolicy, ATTRIBUTES);
}
...
private static auto getPolicyAttribute(TT, string mname, alias Attribute, alias Policy)(
		Attribute!DefaultPolicy default_value) {
	enum val = findFirstUDA!(Attribute!Policy, TypeTuple!(__traits(getMember, TT, mname))[0]);
	static if (val.found)
		return val.value;
	else {
		enum val2 = findFirstUDA!(Attribute!DefaultPolicy,
					TypeTuple!(__traits(getMember, TT, mname))[0]);
		static if (val2.found)
			return val2.value;
		else
			return default_value;
	}
...
private string underscoreStrip(string field_name) @safe nothrow @nogc {
...
private template SerializableFields(COMPOSITE, alias POLICY) {
	alias SerializableFields = FilterSerializableFields!(COMPOSITE, POLICY,
			__traits(allMembers, COMPOSITE));
}

private template FilterSerializableFields(COMPOSITE, alias POLICY, FIELDS...) {
	static if (FIELDS.length > 1) {
		alias FilterSerializableFields = TypeTuple!(FilterSerializableFields!(COMPOSITE, POLICY,
				FIELDS[0 .. $ / 2]), FilterSerializableFields!(COMPOSITE, POLICY, FIELDS[$ / 2 .. $]));
	...
	private struct TestSerializer {
		...
		template unqualSeq(Specs...) {
			static if (Specs.length == 0)
				...
				alias unqualSeq = AliasSeq!(Unqual!(Specs[0]), unqualSeq!(Specs[1 .. $]));
			else
				alias unqualSeq = AliasSeq!(Specs[0], unqualSeq!(Specs[1 .. $]));
		}

		template unqualType(T) {
			static if (isAssociativeArray!T)
				alias unqualType = Unqual!(ValueType!T)[Unqual!(KeyType!T)];
			else static if (isTuple!T)
				alias unqualType = Tuple!(unqualSeq!(TemplateArgsOf!T));
			else static if (isArray!T && !isSomeString!T)
				alias unqualType = Unqual!(ElementType!T)[];
			else
				alias unqualType = Unqual!T;
		}

		string getSerializedResult() @safe {
			return result;
		}

		void beginWriteDictionary(Traits)() {
			result ~= "D(" ~ unqualType!(Traits.Type).mangleof ~ "){";
		}
		...
		void beginWriteDictionaryEntry(Traits)(string name) {
			result ~= "DE(" ~ unqualType!(Traits.Type).mangleof ~ "," ~ name ~ ")(";
		}
		...
		void beginWriteArray(Traits)(size_t length) {
			result ~= "A(" ~ unqualType!(Traits.Type).mangleof ~ ")[" ~ length.to!string ~ "][";
		}

		void endWriteArray(Traits)() {
			result ~= "]A(" ~ unqualType!(Traits.Type).mangleof ~ ")";
		}

		void beginWriteArrayEntry(Traits)(size_t i) {
			result ~= "AE(" ~ unqualType!(Traits.Type).mangleof ~ "," ~ i.to!string ~ ")(";
		}

		void endWriteArrayEntry(Traits)(size_t i) {
			result ~= ")AE(" ~ unqualType!(Traits.Type).mangleof ~ "," ~ i.to!string ~ ")";
		}
...
unittest { // basic serialization behavior
	...
	static void test(T)(auto ref T value, string expected) {
		...
		static if (isPointer!T) {
			if (value)
				assert(*deserialize!(TestSerializer, T)(expected) == *value);
			else
				assert(deserialize!(TestSerializer, T)(expected) is null);
		} else static if (is(T == Nullable!U, U)) {
			if (value.isNull())
				assert(deserialize!(TestSerializer, T)(expected).isNull);
			else
				assert(deserialize!(TestSerializer, T)(expected) == value);
		} else
			assert(deserialize!(TestSerializer, T)(expected) == value);
	...
	test(["hello", "world"],
			"A(AAya)[2][AE(Aya,0)(V(Aya)(hello))AE(Aya,0)AE(Aya,1)(V(Aya)(world))AE(Aya,1)]A(AAya)");
	string mangleOfAA = (string[string]).mangleof;
	test(["hello": "world"],
			"D(" ~ mangleOfAA ~ "){DE(Aya,hello)(V(Aya)(world))DE(Aya,hello)}D(" ~ mangleOfAA ~ ")");
...
unittest { // tuple serialization
	...
	const es = "D(" ~ Sm ~ "){DE(" ~ Tum ~ ",f)(A(" ~ Tum
		~ ")[2][AE(i,0)(V(i)(42))AE(i,0)AE(Aya,1)(V(Aya)(hello))AE(Aya,1)]A(" ~ Tum
		~ "))DE(" ~ Tum ~ ",f)}D(" ~ Sm ~ ")";
	assert(ss == es);
	...
	const et = "D(" ~ Tm ~ "){DE(" ~ Sm ~ ",g)(A(" ~ Sm
		~ ")[2][AE(i,0)(V(i)(42))AE(i,0)AE(Aya,1)(V(Aya)(hello))AE(Aya,1)]A(" ~ Sm
		~ "))DE(" ~ Sm ~ ",g)}D(" ~ Tm ~ ")";
	assert(st == et);
...
unittest { // named tuple serialization
	...
	static struct T {
		@asArray typeof(tuple!(FieldNameTuple!I)(I.init.tupleof)) tuple1AsArray;

		@name(fullyQualifiedName!I)
		typeof(tuple!(FieldNameTuple!I)(I.init.tupleof)) tuple1AsDictionary;

		@asArray typeof(tuple!(FieldNameTuple!S)(S.init.tupleof)) tuple2AsArray;
	...
	const et = "D(" ~ Tm ~ ")" ~ "{" ~ "DE(" ~ TuIm ~ ",tuple1AsArray)" ~ "(" ~ "V(i)(42)"
		~ ")" ~ "DE(" ~ TuIm ~ ",tuple1AsArray)" ~ "DE(" ~ TuIm ~ ","
		~ fullyQualifiedName!I ~ ")" ~ "(" ~ "D(" ~ TuIm ~ ")" ~ "{" ~ "DE(i,i)" ~ "("
		~ "V(i)(42)" ~ ")" ~ "DE(i,i)" ~ "}" ~ "D(" ~ TuIm ~ ")" ~ ")" ~ "DE(" ~ TuIm ~ ","
		~ fullyQualifiedName!I ~ ")" ~ "DE(" ~ TuSm ~ ",tuple2AsArray)" ~ "(" ~ "A(" ~ TuSm
		~ ")[2]" ~ "[" ~ "AE(i,0)" ~ "(" ~ "V(i)(42)" ~ ")" ~ "AE(i,0)" ~ "AE(Aya,1)"
		~ "(" ~ "V(Aya)(hello)" ~ ")" ~ "AE(Aya,1)" ~ "]" ~ "A(" ~ TuSm ~ ")" ~ ")" ~ "DE("
		~ TuSm ~ ",tuple2AsArray)" ~ "DE(" ~ TuSm ~ "," ~ fullyQualifiedName!S ~ ")"
		~ "(" ~ "D(" ~ TuSm ~ ")" ~ "{" ~ "DE(i,x)" ~ "(" ~ "V(i)(42)" ~ ")" ~ "DE(i,x)"
		~ "DE(Aya,s)" ~ "(" ~ "V(Aya)(hello)" ~ ")" ~ "DE(Aya,s)" ~ "}" ~ "D(" ~ TuSm
		~ ")" ~ ")" ~ "DE(" ~ TuSm ~ "," ~ fullyQualifiedName!S ~ ")" ~ "}" ~ "D(" ~ Tm ~ ")";
	assert(st == et);
...
unittest { // testing the various UDAs
	...
	assert(serialize!TestSerializer(s) == "D(" ~ Sm ~ "){DE(" ~ Em
			~ ",e)(V(Aya)(world))DE(" ~ Em ~ ",e)DE(f,f)(V(f)(1))DE(f,f)}D(" ~ Sm ~ ")");
}
...
@safe unittest { // custom serialization support
	...
	static struct S1 {
		...
		static S1 fromString(string) @safe {
			return S1.init;
		}
	}
	...
	static struct C1 {
		int i;
		float toRepresentation() const @safe {
			return 1.0f;
		}

		static C1 fromRepresentation(float f) @safe {
			return C1.init;
		}
	}
	...
	static struct C3 {
		int i;
		static C3 fromRepresentation(float f) {
			return C3.init;
		}
	}
...
unittest  // Testing corner case: Variadic template constructors and methods
{
	...
	static struct S {
		...
		int foo(Args...)(Args args) {
			return i;
		}

		ref int bar(Args...)(Args args) {
			return i;
		}
	}
...
unittest { // test BitFlags serialization
	...
	enum Fac_ser = "A(" ~ Flagsm ~ ")[2][AE(" ~ Flagm ~ ",0)(V(i)(1))AE(" ~ Flagm
		~ ",0)AE(" ~ Flagm ~ ",1)(V(i)(4))AE(" ~ Flagm ~ ",1)]A(" ~ Flagsm ~ ")";
	...
	enum Sm = S.mangleof;
	enum Sac_ser = "D(" ~ Sm ~ "){DE(" ~ Flagsm ~ ",f)(A(" ~ Flagsm ~ ")[2][AE(" ~ Flagm ~ ",0)(V(Aya)(a))AE("
		~ Flagm ~ ",0)AE(" ~ Flagm ~ ",1)(V(Aya)(c))AE(" ~ Flagm ~ ",1)]A(" ~ Flagsm
		~ "))DE(" ~ Flagsm ~ ",f)}D(" ~ Sm ~ ")";
...
@safe unittest { // issue #1182
	...
	enum Sm = S.mangleof;
	enum Tm = T.mangleof;
	enum s_ser = "D(" ~ Sm ~ "){DE(" ~ Tm ~ ",t)(A(" ~ Tm
		~ ")[2][AE(i,0)(V(i)(42))AE(i,0)AE(Aya,1)(V(Aya)(foo))AE(Aya,1)]A(" ~ Tm
		~ "))DE(" ~ Tm ~ ",t)}D(" ~ Sm ~ ")";
...
@safe unittest { // issue #1352 - ingore per policy
	...
	auto t = T(1, 2, 3, 4);
	auto Tm = T.mangleof;
	auto t_ser_plain = "D(" ~ Tm
		~ "){DE(i,b)(V(i)(2))DE(i,b)DE(i,c)(V(i)(3))DE(i,c)DE(i,d)(V(i)(4))DE(i,d)}D(" ~ Tm ~ ")";
...
unittest {
	...
	@safe:
		static string toRepresentation(T v) {
			return v.to!string.toLower();
		}

		static T fromRepresentation(string str) {
			return str.toUpper().to!T;
		}
	}
	...
	assert(serializeWithPolicy!(TestSerializer, P)(E.RED) == ser_red,
			serializeWithPolicy!(TestSerializer, P)(E.RED));
	assert(deserializeWithPolicy!(TestSerializer, P, E)(ser_red) == E.RED);

	import dutils.data.json : JSON, JSONSerializer;
...
unittest {
	...
	@safe:
		int x;
		R toRepresentation() const {
			return R(x);
		}

		static Custom fromRepresentation(R r) {
			return Custom(r.y);
		}
	}
...
@system unittest {
	static struct SystemSerializer {
		...
		T readValue(Traits, T)() @system {
			return ser.readValue!(Traits, T);
		}

		void writeValue(Traits, T)(T value) @system {
			ser.writeValue!(Traits, T)(value);
		}

		void readDictionary(Traits)(scope void delegate(string) @system entry_callback) {
			ser.readDictionary!Traits((s) @trusted { entry_callback(s); });
		}

		void readArray(Traits)(scope void delegate(size_t) @system size_callback,
				scope void delegate() @system entry_callback) {
			ser.readArray!Traits((s) @trusted { size_callback(s); }, () @trusted {
				entry_callback();
			});
		}
	}
...
unittest { // issue 1991 - @system property getters/setters does not compile
	static class A {
		@property @name("foo") {
			@safe string fooString() const {
				return "a";
			}

			@safe void fooString(string a) {
			}
		}
	}
...
unittest { // issue #2110 - single-element tuples
	...
	{
		static struct T {
			@asArray typeof(F.init.tupleof) fields;
		}
