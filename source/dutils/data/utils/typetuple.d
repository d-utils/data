...
module dutils.data.utils.typetuple;
 ...
 */
private template isSame(ab...) if (ab.length == 2) {
	static if (is(ab[0]) && is(ab[1])) {
		enum isSame = is(ab[0] == ab[1]);
	} else static if (!is(ab[0]) && !is(ab[1]) && is(typeof(ab[0] == ab[1]) == bool)
			&& (ab[0] == ab[1])) {
...
*/
template Compare(alias Group1, alias Group2) if (isGroup!Group1 && isGroup!Group2) {
	private template implementation(size_t index) {
		static if (Group1.expand.length != Group2.expand.length)
			enum implementation = false;
		else static if (index >= Group1.expand.length)
			enum implementation = true;
		else static if (!isSame!(Group1.expand[index], Group2.expand[index]))
			enum implementation = false;
		else
			enum implementation = implementation!(index + 1);
	}
